        ...
        for cluster_id in self.cluster_models.keys():
            ...
            for site in cluster_sites:
                ...
                if os.path.exists(parquet_file):
                    try:
                        df = pd.read_parquet(parquet_file, columns=centroid_features)
                        if len(df) > 0:
                            # Use mean values for this site
                            site_means = df.mean().values
            ...
            if cluster_data:
                ...
                print(f"  ✅ Cluster {cluster_id}: Centroid calculated from {len(cluster_data)} sites")
            else:
