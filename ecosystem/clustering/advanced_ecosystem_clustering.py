...
class AdvancedEcosystemClusterer:
    ...
    def __init__(self, data_dir='../../processed_parquet', output_dir='../evaluation/clustering_results', feature_set='advanced'):
        ...
        # Hybrid features (from XGBoost importance + advanced set)
        self.hybrid_numeric = [
            # Geographic/Climate
    ...
    def select_best_strategy(self, strategies, clustering_df):
        ...
        for strategy in balanced_strategies:
            # Combined score: balance_ratio * silhouette_score
            strategy['combined_score'] = strategy['balance_ratio'] * strategy['silhouette']
            
            # Bonus for having more clusters (more granular)
            cluster_bonus = min(strategy['n_clusters'] / 8.0, 1.0)  # Cap at 1.0
            strategy['final_score'] = strategy['combined_score'] * (1 + 0.2 * cluster_bonus)
        ...
        for i, strategy in enumerate(balanced_strategies[:5]):
            print(f"    {i+1}. {strategy['name']}: balance={strategy['balance_ratio']:.3f}, "
                  f"silhouette={strategy['silhouette']:.3f}, clusters={strategy['n_clusters']}, "
                  f"score={strategy['final_score']:.3f}")
