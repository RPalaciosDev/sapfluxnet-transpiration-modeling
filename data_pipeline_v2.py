        ...
        for col in text_columns:
            if col in preserve_cols:
                ...
                if is_likely_identity and unique_count > 5:
                    ...
                    features = features.drop(col, axis=1)
                    skipped_identity_features.append(col)
                    print(f"    🚨 BLOCKED likely identity feature: {col} (pattern: {col_lower})")
                    continue
                ...
                if unique_count <= 10 and not is_likely_identity:
                    ...
                    print(f"    🔍 Evaluating small categorical: {col} ({unique_count} values)")
                    ...
                    if not all(str(val).replace('.', '').replace('-', '').isdigit() for val in sample_values if pd.notna(val)):
                        ...
                        features = features.drop(col, axis=1)
                        encoded_features.append(col)
                        print(f"    ✅ Encoded small categorical: {col} → {col}_code")
                    else:
                        # Looks like numeric IDs - drop it
                        features = features.drop(col, axis=1)
                        skipped_identity_features.append(col)
                        print(f"    🚨 BLOCKED numeric ID pattern: {col}")
                    ...
                    if all(str(val).replace('.', '').replace('-', '').isdigit() for val in sample_values):
                        ...
                        features = features.drop(col, axis=1)
                        skipped_identity_features.append(col)
                        print(f"    🚨 BLOCKED numeric ID pattern: {col} ({unique_count} values)")
                    else:
                        # Could be ecological categorical but high cardinality is risky
                        features = features.drop(col, axis=1)
                        dropped_features.append(col)
                        print(f"    ⚠️  DROPPED high cardinality categorical: {col} ({unique_count} values)")
                else:
                    ...
                    try:
                        ...
                        features = features.drop(col, axis=1)
                        dropped_features.append(col)
                        print(f"    🗑️  DROPPED high cardinality text: {col} ({unique_count} values)")
